...
//var PORT = process.env.PORT || 3000; 1개의 서비스포트로 사용시 지정
module.exports = app;
...
io.on('connection', function(socket) {
    ...
    io.emit('OnOff', jsonMsg);//스프링의 Model같은역할-접속한 All소켓에 OnOff 변수명으로 msg를 보냅니다.
    //client가 접속을 끊었을때
	socket.on('disconnect',function() {
        console.log(socket.id + ' user 접속끊어짐');
    });
	//로컬 PC에서 테스트시 확인은 포트사용 http://localhost:5000/socket.io/socket.io.js 이 소스를 사용
